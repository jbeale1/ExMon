// Read LIS3DH, transmit data words via LoRa
// 13-Sep-2021 JPB
...
// ------------------------------------------------------------
#define HBINS 15            // how many motion histogram bins
...
void loop() {
  ...
  for (int j=0;j<25;j++) {
    ...
    float ld = 2.3*log(dmag);
    int midx = min(max((ld+(1.1*HBINS)),MMIN),MMAX);
  ...
  buf[HBINS]=',';  // separator between motion word and packet index
  itoa(packetnum,(buf + HBINS + 1),10);  // insert packet number
  buf[PKTLEN-1] = 0;  // make sure it's null-terminated
  Serial.println(buf);  // send it out USB port for debug info
