// Read LIS3DH, find spectral peak, transmit data words via LoRa
// 02-Oct-2021 JPB
...
void loop() {
  ...
  for (int i=0;i<samples/2;i++) {
      ...
      delay(40);  // msec delay  25 Hz => 40 msec
  } // for (i...)
  // we just read in half a rawData[] buffer, now run FFT on all of it

  for (int i=0;i<samples;i++) {
    vReal[i] = rawData[i]; // transfer full buffer into FFT buffer
